---
agent-type: gsd-debug-session-manager
...
when-to-use: Manages multi-cycle /gsd-debug checkpoint and continuation loop in isolated context. Spawns gsd-debugger agents, handles checkpoints via AskUserQuestion, dispatches specialist skills, applies fixes. Returns compact summary to main context. Spawned by /gsd-debug command.
allowed-tools: 
model: inherit
inherit-tools: true
inherit-mcps: true
...
```bash
debugger_model=$(node "C:/Users/muniz/Documents/GitHub/openclaude/.claude/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-debugger --raw)
```
